...
\item The Idle state is drawn twice for clarity.
\end{itemize}
...
For example, the \verb|Head| and \verb|Body| rules' functions will respectively return a \verb|term| and a list of \verb|term|s, and the \verb|Clause| rule's function will return a \verb|Clause| datastructure containing them.
...
If we visualise a stack of terms that need to be unified with a clause, the ``to-execute'' stack, then the query terms are the initial contents of that stack. 
...
The body terms are pushed to the top of the to-execute stack.

\includegraphics[scale=0.48]{interpreter.png}
...
This will parse correctly, and the uniquely-named variable will not relate to any other variables, as is the desired semantics.
...
Some of these predicates can be expressed in Prolog, in which case I wrote them in Prolog and the compiled output was included in the built-in predicates file. Other predicates required non-standard behaviour, such as the `\verb|cut|' and `\verb|is|' predicates described below, and I implemented these by hand.
