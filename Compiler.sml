...
fun compileQuery( out, Query( terms ), num ) = 
    ...
    in
        ...
        TextIO.output( out, Int.toString( num ) );
        TextIO.output( out, "() = \n" );
        TextIO.output( out, "    let val timer = Timer.startCPUTimer()\n" );
        TextIO.output( out, "        fun succ( uni, fail ) = (\n" );
        TextIO.output( out, "                print( \"success\\n\" );\n" );
        TextIO.output( out, "                printUnifier( " );
        TextIO.output( out, "substituteUnifier( cleanUnifier( uni ) ) );\n" );
        TextIO.output( out, "                let val {usr, sys} = " );
        TextIO.output( out, "Timer.checkCPUTimer( timer )\n" );
        TextIO.output( out, "                in\n" );
        TextIO.output( out, "                    print " );
        TextIO.output( out, "\"Execution completed in \";\n" );
        TextIO.output( out, "                    print( " );
        TextIO.output( out, "Time.toString( Time.+( usr, sys ) ) );\n" );
        TextIO.output( out, "                    print \" seconds.\\n\"\n" );
        TextIO.output( out, "                end\n" );
        TextIO.output( out, "            )\n" );
